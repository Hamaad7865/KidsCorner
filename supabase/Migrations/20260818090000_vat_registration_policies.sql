...
revoke all on table public.vat_policies from public, anon, authenticated;
grant select on table public.vat_policies to authenticated;
grant all on table public.vat_policies to service_role;
...
do $$
declare
    v_saved_rate_text text;
    v_saved_number text;
    v_configured_rate numeric(7,6) := 0.15;
    v_vat_number text;
    v_legacy_id bigint;
begin
    select value #>> '{}'
      into v_saved_rate_text
      from public.settings
     where key = 'vat_rate';

    if v_saved_rate_text is not null
       and btrim(v_saved_rate_text) ~ '^[+]?[0-9]+([.][0-9]+)?$'
       and btrim(v_saved_rate_text)::numeric > 0
       and btrim(v_saved_rate_text)::numeric <= 1 then
        v_configured_rate := btrim(v_saved_rate_text)::numeric(7,6);
    end if;

    select value #>> '{}'
      into v_saved_number
      from public.settings
     where key = 'vat_number';
    v_vat_number := nullif(btrim(v_saved_number), '');
